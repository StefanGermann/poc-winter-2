...
A browser-based simulation tool for the Adelboden 2026/27 ski-pass POC pricing model. Users adjust max_EBD, adoption level, and product shares interactively to see w1, w2_old, and Total_Effect outputs. All model logic runs entirely in the browser – no backend or database. Built with TanStack Start and deployed on Netlify.

## Model Logic (src/components/Simulator.tsx)

The Excel POC sheet is re-implemented in pure TypeScript:

1. **LOOKUP_TABLE** – hard-coded `(demand, avg_old)` pairs from columns M/O (rows 8–108). Used by `vlookup()` for approximate-match lookups.
2. **`getEarlyBookers(adoption)`** – computes the 15-element early-booker rates from a base adoption value via the cascading MIN/multiply formulas in the Excel.
3. **`w1[i]`** = `max_EBD * ((1 - vlookup(eb[i])) * eb[i] + 0.2 * lb[i])` – product-specific weight.
4. **`w2_old[i]`** = same but uses portfolio-level EB rate `SUMPRODUCT(shares, earlyBookers)`.
5. **Total_Effect (J6)** = `SUM((w2_old - w1) * share)`.

Inputs: `maxEBDPct` (0–100), `adoption` (weak/medium/strong/very strong), `shares[]` (15 floats in %, sum should be 100).
